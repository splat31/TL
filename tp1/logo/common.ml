...
(** Used to escape double-quotes. *)
let escape_quote_re = Str.regexp_string "\""


(** Escape required character for JSON string.
	@param str	String to replace in.
	@return		Escaped string. *)
let escape_string str =
	Str.global_replace escape_quote_re "\\\"" str
...
let json_to_string j =
	...
	and trans j =
		...
		| JARRAY l ->
			...
			| h::t ->
				trans h;
				List.iter (fun j -> addc ','; trans j) t);
			addc ']'
		| JRECORD rs ->
...
(** Map the element of the array to a list.
	@param f	Function to apply.
	@param arr	Array to traversE.
	@param n	Index to stop to.
	@return		List after map. *)
let map_array_to f arr n =
	let rec process i =
		if i >= n then [] else
		(f arr.(i)) :: (process (i + 1)) in
	process 0
...
(** Map the element of the array to a list.
	...
	@return		List after map. *)
let map_array f arr =
	map_array_to f arr (Array.length arr)


(** Iteration from 0 to n.
	Map from [i = 0, n-1] to [f i]).
	@param f	Function to transform i into a value.
	@param n	Vaule to count to.
	@return		Map of each i in [0, n-1]. *)
let map_n f n =
	let rec work i =
		if i == n then [] else
		(f i)::(work (i+1)) in
